...
/**
 * with talisman wallet
 */

const testWithTalisman = createWalletTest({
  wallets: [{ type: 'talisman' }],
})

testWithTalisman.beforeAll(async ({ wallets }) => {
...
testWithTalisman('sign transaction on polkadot starter with talisman wallet', async ({ page, wallets }) => {
  await page.goto(POLKADOT_DAPP_URL)
  await page.waitForLoadState('networkidle')
  ...
  await page.getByText('Processing transaction...').waitFor({ state: 'visible' })
  await page.getByText('Processing transaction...').isVisible()
})
